...
export class CrudEjemploActualizarComponent {
  ...
  objUsuario: Usuario = {};
   ...
   constructor(private ejemploService: EjemploService,
            ...
            @Inject(MAT_DIALOG_DATA) public data: Ejemplo) {
   ...
   actualiza() {
        ...
        console.log(">>> actualiza [inicio] ", this.ejemplo);


        this.ejemploService.actualizarCrud(this.ejemplo).subscribe(
          x=>{
  ...
  validaDescripcion(control: FormControl) {
    ...
    return this.ejemploService.validaDescripcionActualiza(control.value, this.ejemplo.idEjemplo || 0).pipe(
       map((resp: any) => { 
